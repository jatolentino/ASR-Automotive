...
#     configs = BaseModelConfigs.load("Models/05_sound_to_text/202503210353/configs.yaml")

#     model = WavToTextModel(model_path=configs.model_path, char_list=configs.vocab, force_cpu=False)

#     df = pd.read_csv("Models/05_sound_to_text/202503210353/val.csv").values.tolist()
...
if __name__ == "__main__":
    ...
    # You can change this to any row index to pick a single sample
    df = pd.read_csv("Models/05_sound_to_text/202503210353/val.csv").values.tolist()
    wav_path, label = df[7]  # Pick the first sample, or you can choose another index
