...
# Comment this block and move the datasets generated with the wav files generated with synthetic_data/main.py
# And put it on a folder: ASR/Datasets/LJSpeech-1.1/wavs
...
# Adjust column names based on the new structure: file_name and transcription
metadata_df.columns = ["file_name", "transcription"]

# Optional: Add a "normalized_transcription" column (if needed for your task)
metadata_df["normalized_transcription"] = metadata_df["transcription"].str.lower()

# Structure the dataset where each row is a list of [wav_file_path, sound transcription]
dataset = [
    [f"Datasets/LJSpeech-1.1/wavs/{file}.wav", label.lower()] 
    for file, label in metadata_df[["file_name", "normalized_transcription"]].values.tolist()
]
